...
**为什么没有顺手把兜底降权**：实测全部数据集的 274 条真实消息，**31% 由这条兜底定下**，
降权会让模型调用率从 30% 翻倍到约 60%，而它们绝大多数本来就是课程问答（「ANOVA 的假设条件有哪些？」）。
更省的候选是只对**第一人称自指**的疑问句降权 —— 85 条兜底里只有 11 条含「我」（12.9%），
且恰好覆盖全部失败用例。该修复尚未实施，记为 K-39。
