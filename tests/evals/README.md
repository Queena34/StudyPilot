...
| fallback_rate | 0% |
| **score_band_accuracy** | **100%** |
| average_latency_ms | 2340 |

旧基线曾只有 90%，9 次越界全部集中在 `grade-003/004/005` 的 partial 档。诊断发现根因不是“简短答案应扣分”，而是批改器拿整道题约束每一条 rubric，导致已覆盖要点也被重复扣分。改为逐条独立判定 rubric 后，90 次完整重跑的分数区间准确率恢复到 100%。
...
基线**如实记录当前行为，未通过修改数据集抬高数值**。2026-09-01 再次完整重跑仍保持分数区间、排序和重复稳定性全部 100%；错误答案平均分 2.5，仍稳定落在预设的 0–29.99 区间。
...
- `composite_supporting_accuracy` 衡量复合意图的辅助 Agent 识别；2026-09-02 的 9 个复合场景达到 100%，作为“后续步骤不得被静默丢弃”的回归门槛。
...
```bash
# 1. 对 RAG v1 全部 30 题生成真实回答，并抓取模型实际看到的引用片段
python -m tests.evals.run_faithfulness_sample --course-id <课程ID> --size 30
...
评审页显示的是**模型实际看到的完整引用片段**。v2 不再为了“取完整片段”执行第二次检索：原回答可能使用翻译后的查询和章节过滤，第二次普通检索会取回不同片段，反而让评审工具错误标记引用。抽样器还会检查分块上限不得超过引用片段上限，否则直接失败。

**评审工具让正确的行为看起来是错的，比没有评审更糟。** v1 首轮曾因只展示 300 字符，把正确引用误判为错误；该历史保留在 `baselines/faithfulness_v1.json`，但 v2 已取代 v1 成为当前回归基线。
...
`baselines/faithfulness_v2.json` 记录评审者 Queena 对 RAG v1 全部 30 题的人工判断：
...
| fabrication_rate | 0% |
| declined_when_unsupported_rate | 100% |

其中 25 题可回答、5 题资料外，覆盖 12 种问题层。五条资料外题的复选框最初漏勾；评审者书面确认回答已明确说明资料未涵盖后，只更正了对应 `admits_gap` 字段。30 条全部通过仍不能外推为任意课程上的真实忠实度都是 100%；按经验法则，30 次零失败时真实失败率的 95% 上界约为 10%。
...
## Paraphrase v1（同一意图的多种说法）

- 数据集：`datasets/router_paraphrase_v1.jsonl`，**11 组、41 条**。每组是同一个请求的 3–5 种自然说法。
- 运行器：`run_paraphrase_eval.py`，与路由评测一样进程内调用，无需起 API。

**首要指标是「组内一致性」，不是准确率。** 同组每条说的是同一件事，路由给出不同答案就是缺陷 ——
哪怕其中几个答案是对的。学习者换个说法就得到不同行为，那是 bug，不是偏好。同时并列记录
`agent_group_consistency`：两个意图标签可能指向同一个 Agent，混淆它们扣一致性却不改变学习者拿到的结果。

**为什么需要它。** 路由评测集问的是「这一种说法能不能懂」，本评测问的是「换个说法还懂不懂」。
后者是前者看不见的：两种复合触发方案在 57 条路由用例上三轮得分**完全相同**，只有词表之外的说法
才把它们分开。
...
| 指标 | 结果 |
|---|---:|
| 组内一致性 | **45.5%**（11 组中 6 组不一致） |
| Agent 层组内一致性 | 54.5% |
| 复合意图组一致性 | 100% |
| 意图准确率 | **80.5%**（路由评测集上是 94.2%） |

意图准确率比路由评测集低 13.7 个百分点，**证实 57 条用例高估了泛化能力**。

**根因**：9 个失败中 8 个由规则层判成 `course_qa` 且**未调用模型**。疑问句兜底的置信度是 0.85，
高于 0.80 的阈值，所以只要句子长得像问句就直接锁定 `course_qa`。但此时系统只知道「这是个问句」，
并不知道问的是什么 —— **确定的是形式，不是意图**。典型误判：

- 「哪些知识点我还不熟」「我学得如何了」→ 应为查进度
- 「我该怎么安排复习」→ 应为排计划
- 「我上传了哪些资料」→ 应为资料清单

**修复（2026-09-01）**：把疑问句兜底的置信度从 0.85 降到 0.60，让它交给模型判断。

一度考虑过更省的做法 —— 只对**第一人称自指**的疑问句降权（85 条兜底里只有 11 条含「我」）。
但那仍然是穷举，而且它成立的前提是「我编的那些说法代表真实用户」，无从证明。
**「一条规则都没匹配上」本身就意味着系统不知道这是什么，不知道就该问模型** —— 这条原则不依赖任何词表。

先估的代价是错的：按「31% 的消息走这条兜底」外推，以为调用率会翻倍到 60%，
实测只是 **0.298 → 0.404**，因为其中很多消息本来就会因别的原因走模型。

| 指标 | 改前 | 改后 |
|---|---:|---:|
| 组内一致性 | 45.5% | **81.8%** |
| 同义表达意图准确率 | 80.5% | **95.1%** |
| 同义表达 Agent 准确率 | 85.4% | **95.1%** |
| 路由 Agent 准确率 | 98.1% | **98.1%** |
| 路由意图准确率 | 94.2% | 86.5% |
| 模型调用率 | 29.8% | 40.4% |

**路由意图准确率下降 7.7 点，但 Agent 准确率一点没动。** 7 个新增失配里 6 个是
`course_qa` 与 `concept_explanation` 互判 —— 二者同属 TutorAgent，学习者拿到的结果一样；
唯一真实的路由错误改动前也存在。换言之，**这次下降没有一分是新增的用户可感知问题**，
而一致性的提升是实打实的。度量要对准用户结果，所以接受这次交换。

剩余 2 组不一致（「测测我」被判为闲聊、「帮我看看答得对不对」被判为课程问答）是模型自身的判断错误，
不再是规则短路，需要改提示词，记为 K-40。
...
因此**修复方向不是放宽评分标准**（near_miss 零泄漏说明它并不松），而是让批改器逐条独立评估。该修复已经完成；Grading v1 的 90 次完整重跑中，分数区间、排序一致性与重复稳定性均为 1.0。
