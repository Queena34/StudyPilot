...
| 对应版本 | commit `ff96990` |
| 核对日期 | 2026-08-27 |
| 规模 | `app/` 7557 行，`tests/` 6714 行，277 个单元测试，10 套离线评测 |
...
五种降级原因各有专属文案，未知原因**不套用任何一条具体说法** —— 把未知情形说成「请求失败」是在陈述不成立的事实，也会把排查引向错误方向：

| 原因 | 含义 |
|---|---|
| `model_unconfigured` | 没有配置模型密钥 |
| `provider_request_failed` | 调用模型出错 |
| `empty_model_response` | 模型返回空内容 |
| `citation_validation_failed` | 首次作答无引用 |
| `citation_retry_failed` | 重试后仍无引用 |

Web 端为降级回答加**橙色徽章 + 一句原因说明**，历史回放时按 `model_name == "retrieval-fallback"` 同样标出（存储的消息不保留具体原因）。降级回答读起来和正常回答一样，才是真正误导人的地方。
...
277 个，位于 `tests/unit/`。覆盖路由、编排、工具层权限、诚信 Guard、注入抵抗判定、Skill 选择、分块、解析、检索、语言检测与查询翻译、引用修复重试、出题校验、批改、掌握度、计划、偏好、Web 契约、说明书与代码的一致性。
