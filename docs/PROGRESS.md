...
| K-17 | 批改器偏保守：原判断为「对措辞简短的 rubric 条目只给 0.5」 | 中 | **成因已查明，说法被推翻**（与长度无关；真因是按整道题而非逐条评估，见 `grading-brevity-v1`）。修复方案已定，未实施 |
...
| K-32 | 批改器按「整道题」评估答案，而非逐条独立评估 rubric。答案只覆盖题目一半时，它在**已覆盖**的那条上也只拿部分分。受控实验：同一答案与 rubric，仅把题目改成只问该要点，得分由 0.00/0.50 升至 1.00（三次重复一致） | 中 | 未修复 |
| K-33 | 批改器对含糊措辞扣分（hedged 平均 0.333 vs 清晰表述 0.600）。学生表述不确定但要点正确时会被低估 | 低 | 未修复 |
