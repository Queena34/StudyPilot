...
| K-19 | 全部评测集仍缺人工忠实度抽检，当前指标都是可自动判定的代理指标 | 中 | **流程已就绪**；首轮评审因工具缺陷作废，已修复并重新抽样，**等待第二轮评审** |
| K-22 | Citation 的 `snippet` 只有 300 字符，而片段中位长度约 2000 字符。学习者在 Web 端展开引用时看到的同样是 15% 的内容，可能不足以核实一条论断（可点击跳转 PDF 页作为补偿） | 中 | 未修复 |
| K-21 | 不指定资料时，「根据资料第一章…」这类问题会跨两份资料检索并返回「证据不足」，尽管系统具备章节解析能力。理想行为是反问指哪一份，而不是直接放弃 | 中 | 未修复 |
...
- **怎么验证的**：
  ...
  - 单元测试 `209 passed`。
- **下一步**：**需要第二轮人工评审**。题目与第一轮相同（种子未变），但这次能看到完整的引用原文。
