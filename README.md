...
**当前运行栈**：FastAPI + PostgreSQL + ChromaDB + Docker。后端入口 `app.main:app`，API 统一 `/api/v1` 前缀；前端为 FastAPI 托管的原生 HTML/CSS/JS 工作台。Compose 仍启动 Redis 和 Prometheus，但 Redis 尚未接入应用，Prometheus 当前只能抓取 Python 运行时默认指标；二者不能算作已完成的产品能力。
