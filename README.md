...
- **规则优先。** 最常见的明确请求由确定性规则零延迟解决，实测 LLM 只在约 30% 的路由用例上被调用。
...
| 评测集 | 规模 | 当前基线 | 需要模型 |
|---|---|---|---|
| Router v2 | 57 例 | 意图准确率 94.2%、Agent 准确率 98.1%、澄清准确率 100%，范围保持 100% | 部分 |
...
| Paraphrase v1 | 11 组 41 条 | 组内一致性 45.5%（新增，用于暴露换种说法后的判断漂移） | 部分 |
